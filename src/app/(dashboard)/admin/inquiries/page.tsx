    ...
    const fetchInquiries = async () => {
        try {
            ...
            const response = await fetch(
                `/api/inquiry?search=${searchQuery}&page=${currentPage}&limit=${inquiriesPerPage}&startDate=${startDate}&endDate=${endDate}`
            );
                                ...
                                <tbody className="divide-y divide-gray-200 bg-white">
                                    {inquiries.map((enq) => (
                                        <tr key={enq._id} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${enq.status === 'New' ? 'bg-blue-50 text-blue-600 border-blue-100' :
                                                    enq.status === 'Contacted' ? 'bg-amber-50 text-amber-600 border-amber-100' :
                                                        enq.status === 'Enrolled' ? 'bg-green-50 text-green-600 border-green-100' :
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button onClick={() => handleDelete(enq._id, enq.name)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete Inquiry"><Trash2 size={18} /></button>
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                        ...
                        <div className="p-8 space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-6 rounded-2xl border border-gray-100">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-sm font-semibold text-gray-900">{selectedInquiry.course_name}</p>
                                </div>
                                <div className="space-y-1">
