...
// Types are now imported from @/types/entrance-exam
...
function EntranceExamContent() {
  ...
  const [testInfo, setTestInfo] = useState<EntranceTestInfo | null>(null);
  ...
  const { data: testInfoData, isLoading: testInfoLoading, error: testInfoError } = useGetEntranceTestInstructionQuery({ sessionId, testId, collegeId }, { skip: !sessionId || !testId || !collegeId });
  ...
  const handleAutoSubmit = useCallback(async () => {
    try {
      ...
      const res = await submitTest({ sessionId, answers }).unwrap();
      setResult(res);
      setStep("result");
  ...
  const handleStartTest = async () => {
    try {
      const res = await startSession({ sessionId, testId, collegeId }).unwrap();
...
export default function EntranceExamPage() {
  return (
    <div className="fixed inset-0 z-10 overflow-y-auto bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
